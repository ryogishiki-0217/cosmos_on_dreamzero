...
class WANPolicyHead(ActionHead):
    ...
    def encode_image(self, image, num_frames, height, width):
        # 蒸馏时可用 COSMOS_TEACHER_FP32=1 强制 float32，避免 bfloat16 产生 NaN
        _dtype = torch.float32 if os.environ.get("COSMOS_TEACHER_FP32") == "1" else torch.bfloat16
        with torch.amp.autocast(dtype=_dtype, device_type=torch.device(self._device).type):
    ...
    def forward(self, backbone_output: BatchFeature, action_input: BatchFeature) -> BatchFeature:
        ...
        # with open(os.environ.get("COSMOS_DEBUG_SHAPE_FILE", "/workspace/debug.txt"), "a") as _f:
        #     _f.write(...)
        # print("videos", videos.shape)  # 注释掉避免训练时刷屏；需要时再打开
        ...
        # 已注释：原写入 /workspace/debug.txt
        # with open(..., "a") as _f: _f.write(... after encode_video ...)
        ...
        # 已注释：原写入 /workspace/debug.txt
        # with open(..., "a") as _f: _f.write(... after encode_image ...)
        ...
        with open(os.environ.get("COSMOS_DEBUG_SHAPE_FILE", "/workspace/debug.txt"), "a") as _f:
            _f.write(f"[dreamzero/groot/vla/model/dreamzero/action_head/wan_flow_matching_action_tf_efficient_weighted.py] WANPolicyHead.forward (after transpose): noise.shape={tuple(noise.shape)}, latents.shape={tuple(latents.shape)}\n")
            _f.flush()
        ...
        # 已注释：原写入 /workspace/debug.txt
        # with open(..., "a") as _f: _f.write(... seq_len calc ...)
        ...
        # Compute loss（蒸馏时 COSMOS_TEACHER_FP32=1 则用 float32，避免 CausalWanModel 在 bfloat16 下 NaN）
        _dtype = torch.float32 if os.environ.get("COSMOS_TEACHER_FP32") == "1" else torch.bfloat16
        with torch.amp.autocast(dtype=_dtype, device_type=torch.device(self._device).type):
            if actions.numel() > 0:
                _noisy_lat = noisy_latents.transpose(1, 2)
                # 已注释：原写入 /workspace/debug.txt
                # with open(..., "a") as _f: _f.write(... before self.model ...)
            ...
            if actions.numel() > 0:
                ...
                # action_loss_per_sample = has_real_action[:, None].float() * action_loss_per_sample
                # [MODIFIED 2026-03-23] Reason:
                # old broadcast could mismatch last dimension (e.g., 16 vs 32) when has_real_action
                # is [B, T] and action_loss_per_sample is [B, T, D].
                has_real_action_weight = has_real_action.float()
                while has_real_action_weight.ndim < action_loss_per_sample.ndim:
                    has_real_action_weight = has_real_action_weight.unsqueeze(-1)
                # [MODIFIED 2026-03-23] Reason:
                # has_real_action temporal length may be shorter than action loss temporal length
                # (e.g., 16 vs 96) after action sequence expansion in teacher forward.
                # Align by repeat_interleave when target is an integer multiple.
                for dim in range(1, action_loss_per_sample.ndim):
                    src = has_real_action_weight.shape[dim]
                    tgt = action_loss_per_sample.shape[dim]
                    if src == tgt or src == 1:
                        continue
                    if tgt % src == 0:
                        has_real_action_weight = has_real_action_weight.repeat_interleave(tgt // src, dim=dim)
                    else:
                        raise RuntimeError(
                            f"has_real_action shape {tuple(has_real_action_weight.shape)} cannot align to "
                            f"action_loss_per_sample shape {tuple(action_loss_per_sample.shape)} at dim={dim}"
                        )
                action_loss_per_sample = has_real_action_weight * action_loss_per_sample  # apply has_real_action
