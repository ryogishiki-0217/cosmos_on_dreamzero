...
def _map_structure(fn: Callable[[Any], Any], structure: Any) -> Any:
    """
    Minimal substitute for dm-tree's tree.map_structure (no extra dependency).
    Recurses into Mapping (e.g. dict, BatchFeature/UserDict) and list/tuple; leaves get fn(x).
    """
    if isinstance(structure, Mapping) and not isinstance(structure, (str, bytes)):
        ctor = type(structure)
        try:
            return ctor({k: _map_structure(fn, v) for k, v in structure.items()})
        except TypeError:
            return {k: _map_structure(fn, v) for k, v in structure.items()}
    if isinstance(structure, (list, tuple)):
        mapped = [_map_structure(fn, x) for x in structure]
        return type(structure)(mapped)
    return fn(structure)
...
class VLA(PreTrainedModel):
    ...
    def from_pretrained(
        ...
        # Load config: use config_path if provided, else checkpoint dir / config.json
        print("loading config@@")
        if config_path:
            _config_path = config_path if config_path.endswith(".json") else os.path.join(config_path, "config.json")
        else:
            _config_path = os.path.join(pretrained_model_name_or_path, "config.json")
        with open(_config_path, "r") as f:
            config_dict = json.load(f)
        config = VLAConfig(**config_dict)
        print("loading model")
        # 若设置环境变量 WAN21_LOCAL_DIR，将 config 中指向 Wan2.1 基座的路径改为该本地目录，避免无网时从 HuggingFace 下载
        wan21_local = os.environ.get("WAN21_LOCAL_DIR", "").strip()
        if wan21_local and os.path.isdir(wan21_local):
            ac = config.action_head_cfg.get("config") or {}
            if isinstance(ac, dict):
                if "text_encoder_cfg" in ac and isinstance(ac["text_encoder_cfg"], dict):
                    ac["text_encoder_cfg"]["text_encoder_pretrained_path"] = os.path.join(wan21_local, "models_t5_umt5-xxl-enc-bf16.pth")
                if "image_encoder_cfg" in ac and isinstance(ac["image_encoder_cfg"], dict):
                    ac["image_encoder_cfg"]["image_encoder_pretrained_path"] = os.path.join(wan21_local, "models_clip_open-clip-xlm-roberta-large-vit-huge-14.pth")
                if "vae_cfg" in ac and isinstance(ac["vae_cfg"], dict):
                    ac["vae_cfg"]["vae_pretrained_path"] = os.path.join(wan21_local, "Wan2.1_VAE.pth")
                if "diffusion_model_cfg" in ac and isinstance(ac["diffusion_model_cfg"], dict):
                    ac["diffusion_model_cfg"]["diffusion_model_pretrained_path"] = wan21_local
            print("Using WAN21 base from WAN21_LOCAL_DIR:", wan21_local)
