...
class HuggingfaceTokenizer:
    ...
    def __init__(self, name, seq_len=None, clean=None, **kwargs):
        ...
        # 当 config 里是训练机本地路径（如 /mnt/aws-lfs-02/...）且不存在时，用 WAN21_LOCAL_DIR/umt5-xxl 作为本地 tokenizer 目录（需事先下载 google/umt5-xxl 到该目录）
        if os.path.isabs(name) and not os.path.exists(name):
            wan21 = os.environ.get("WAN21_LOCAL_DIR", "").strip()
            if wan21:
                alt = os.path.join(wan21, "umt5-xxl")
