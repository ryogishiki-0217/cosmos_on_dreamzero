...
class LeRobotSingleUniformDataset(Dataset):
    """
    Base dataset class for LeRobot that supports sharding.
    """
    ...
    def _get_lerobot_modality_meta(self) -> LeRobotModalityMetadata:
        ...
        if self.use_global_metadata:
            ...
            print("modality_meta_path @@", modality_meta_path)
            assert (
                modality_meta_path.exists()
            ), f"Please provide a {Path(LE_ROBOT_MODALITY_FILENAME).name} file in {METADATA_DIR / self.tag.value / self.metadata_version}"
            with open(modality_meta_path, "r") as f:
    ...
    def _get_metadata(self) -> DatasetMetadata:
        ...
        # 1.2. Video modalities
        le_info_path = self.dataset_path / LE_ROBOT_INFO_FILENAME
        print("le_info_path", le_info_path)
        assert (
            le_info_path.exists()
        ), f"Please provide a {LE_ROBOT_INFO_FILENAME} file in {self.dataset_path}"
        with open(le_info_path, "r") as f:
    ...
    def _check_integrity(self):
        ...
        for modality_config in self.modality_configs.values():
            for key in modality_config.modality_keys:
                if key == "lapa_action" or key == "dream_actions" or "rl_info" in key:
    ...
    def get_rl_info(
        ...
        """Get the reward data for a trajectory by a base index.
        If the step indices are out of range, pad with the data:
            if the data is stored in absolute format, pad with the first or last step data;
            otherwise, pad with zero.
    ...
    def get_language(
        ...
        if all(isinstance(x, int) for x in task_indices_or_descriptions):
            return self.tasks.loc[task_indices_or_descriptions]["task"].tolist()
        else:
            return task_indices_or_descriptions
    ...
    def get_data_by_modality(
        ...
        if modality == "video":
            ...
            return self.get_lapa_action(trajectory_id, key, base_index)
        elif modality == "dream_actions":
            return self.get_dream_actions(trajectory_id, key, base_index)
        elif modality == "rl_info":
            return self.get_rl_info(trajectory_id, modality, key, base_index)
        else:
            raise ValueError(f"Invalid modality: {modality}")
...
class LeRobotMixtureDataset(Dataset):
    ...
    def from_mixture_spec(
        ...
        assert issubclass(
            dataset_class, (LeRobotSingleDataset, LeRobotSingleUniformDataset)
        ), f"{dataset_class} is not a subclass of LeRobotSingleDataset or LeRobotSingleUniformDataset"
        data_mixture = []
