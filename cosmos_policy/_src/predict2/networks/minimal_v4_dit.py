...
class Attention(nn.Module):
    ...
    def compute_qkv(self, x, context=None, rope_emb=None) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        # Some training/inference paths may keep projection weights in fp32 while activations are bf16/fp16 (or vice versa).
        # Align input dtype with projection weight dtype to avoid F.linear matmul dtype mismatch.
        if x.dtype != self.q_proj.weight.dtype:
            x = x.to(dtype=self.q_proj.weight.dtype)
        q = self.q_proj(x)
        context = x if context is None else context
        if context.dtype != self.k_proj.weight.dtype:
            context = context.to(dtype=self.k_proj.weight.dtype)
        k = self.k_proj(context)
        if context.dtype != self.v_proj.weight.dtype:
            context = context.to(dtype=self.v_proj.weight.dtype)
    ...
    def compute_attention(
        ...
        # Some training/inference paths may keep projection weights in fp32 while activations are bf16/fp16.
        # Align dtype to avoid F.linear matmul dtype mismatch.
        if result.dtype != self.output_proj.weight.dtype:
            result = result.to(dtype=self.output_proj.weight.dtype)
...
class I2VCrossAttention(Attention):
    ...
    def compute_qkv(
        ...
        if img_context.dtype != self.k_img.weight.dtype:
            img_context = img_context.to(dtype=self.k_img.weight.dtype)
        k_img = self.k_img(img_context)
        if img_context.dtype != self.v_img.weight.dtype:
            img_context = img_context.to(dtype=self.v_img.weight.dtype)
    ...
    def compute_attention(self, q, k, v, k_img, v_img):
        ...
        out = result + result_img
        if out.dtype != self.output_proj.weight.dtype:
            out = out.to(dtype=self.output_proj.weight.dtype)
...
class TimestepEmbedding(nn.Module):
    ...
    def forward(self, sample: torch.Tensor) -> torch.Tensor:
        # Ensure dtype match with Linear weights.
        # Some inference paths may provide float32 timesteps while the checkpoint weights are bf16/fp16.
